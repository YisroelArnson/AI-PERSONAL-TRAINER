...
const communicationTools = [
      ...
      description: "Send a message to user without requiring a response. Use for acknowledging receipt, providing progress updates, reporting completion, or explaining strategy changes.",
          parameters: {
            type: "object",
            properties: {
              text: {
            ...
            type: "array",
            items: { type: "string" },
            description: "(Optional) Data attachments to show (e.g., exercise list, stats summary)"
          }
        },
        required: ["text"]
      }
...
When the agent creates exercises via `generate_workout`, the system assigns a unique ID to each exercise. These IDs are:

- Generated by the API (not the agent)
- Returned in the tool result and displayed to the user
- Visible in context when the user requests changes
- Used by modification tools to identify specific exercises
- Consistent across current workout and historical exercises

```
Flow:
1. Agent calls generate_workout with exercises (no IDs)
2. API creates exercises and assigns IDs
3. Result includes exercises with IDs → displayed to user
4. User: "Make the bench press heavier"
5. Agent sees exercises with IDs in context
6. Agent calls adjust_exercise with exercise_id: "8680123"
...
"workout": {
"exercises": [
{ "id": "ex1", "name": "Dumbbell Bench Press", "type": "strength", "sets": 3, "reps": 10, "load_kg": 20 },
{ "id": "ex2", "name": "Bent Over Rows", "type": "strength", "sets": 3, "reps": 10, "load_kg": 18 },
{ "id": "ex3", "name": "Shoulder Press", "type": "strength", "sets": 3, "reps": 10, "load_kg": 12 },
{ "id": "ex4", "name": "Bicep Curls", "type": "strength", "sets": 2, "reps": 12, "load_kg": 8 },
{ "id": "ex5", "name": "Tricep Dips", "type": "bodyweight", "sets": 2, "reps": 10 }
],
...
data: { "type": "message", "content": "Got it! Let me..." }

data: { "type": "tool_start", "tool": "generate_workout" }

data: { "type": "exercise", "data": { "name": "Bench Press", ... } }

data: { "type": "tool_end", "tool": "generate_workout" }

data: { "type": "message", "content": "Here's your workout!" }
